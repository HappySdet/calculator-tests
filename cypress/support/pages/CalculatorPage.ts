...
export class CalculatorPage {
  ...
  clickCalculatorButton(button: string): this {
    cy.get(CALCULATOR_SELECTORS.buttons).contains(button).should('be.visible').click();
    return this;
  }
  ...
  */
  inputThroughKeyboard(input: string): this {
    input.split('').forEach(char => cy.get(CALCULATOR_SELECTORS.textField).type(char)) 
    return this;
  }

  addInput(expression: string, method: 'buttons' | 'keyboard' = 'buttons'): this {
    if (method === 'buttons') {
  ...
  waitForCalculatorToLoad(): this {
    ...
    cy.get(CALCULATOR_SELECTORS.textField).should('be.visible').and('contain.text', '0');
    return this;
  }
